...
def get_embeddings(resume_text, resume_id):
    ...
    try:
        ...
        logger.error(f"Error with vector storage: {str(e)}")
        raise ValueError(f"Failed to create vector store: {str(e)}")

@timing_decorator
def clear_vector_data(resume_id, index_name=None):
    """Clear existing vectors from Pinecone"""
    if not index_name:
        index_name = settings.INDEX_NAME or os.environ.get('INDEX_NAME')
        if not index_name:
            logger.error("No index name provided to clear vector data")
            raise ValueError("Index name must be provided for Pinecone")
    ...
    try:
        ...
        raise ValueError(f"Failed to clear vector data: {str(e)}")

# Keep the old function for backward compatibility
@timing_decorator
def clear_pinecone_data(index_name, namespace=None):
    """Clear existing vectors from Pinecone index (legacy function)"""
    logger.warning("clear_pinecone_data is deprecated, use clear_vector_data instead")
    if namespace and namespace.startswith("resume_"):
        resume_id = namespace.replace("resume_", "")
        return clear_vector_data(resume_id, index_name)

    # Fall back to old implementation for non-resume namespaces
    if not index_name:
...
def query_resume(query, vector_store, chat_history, model="llama3-8b-8192", temperature=0.0, max_tokens=1000, resume_id=None):
    ...
    try:
        ...
        # Set up the retriever with the namespace
