...
if db_url:
    ...
    # Print a sanitized version (hiding password)
    try:
        import urllib.parse
        parsed_url = urllib.parse.urlparse(db_url)
        userpass, hostport = parsed_url.netloc.split('@', 1)
        username, _ = userpass.split(':', 1)
        sanitized_url = f"{parsed_url.scheme}://{username}:*****@{hostport}{parsed_url.path}"
        print(f"Sanitized DATABASE_URL: {sanitized_url}")
    except Exception as e:
        print(f"Error sanitizing DATABASE_URL: {str(e)}")
...
if os.environ.get('RENDER'):
    ...
    # Local development - also use DATABASE_URL
